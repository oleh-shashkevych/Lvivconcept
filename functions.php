...
if (!function_exists('add_scripts')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_scripts() { // добавление скриптов
	    ...
	    wp_enqueue_script('jquery', get_template_directory_uri().'/js/jquery.js','','',true); // добавляем свой
	    wp_enqueue_script('slider', get_template_directory_uri().'/js/slider.js','','',true); // Основной
	    wp_enqueue_script('main', get_template_directory_uri().'/js/main.js','','',true); // Основной
	}
}
...
if (!function_exists('add_styles')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_styles() { // добавление стилей
	    if(is_admin()) return false; // если мы в админке - ничего не делаем
		wp_enqueue_style( 'slidercss', get_template_directory_uri().'/css/slider.css' ); // новые стили шаблона
...
// Disable EWWW Image Optimizer lazy load
add_filter('ewww_image_optimizer_lazy_load', '__return_false');

// Отключение встроенного lazy load в WordPress
add_filter('wp_lazy_loading_enabled', '__return_false');

function disable_lazy_load_js() {
    echo '<script>
...
add_action('wp_head', 'disable_lazy_load_js');

function replace_img_src_with_data_src() {
    echo '<script>
        document.addEventListener("DOMContentLoaded", function() {
...
add_action('wp_footer', 'replace_img_src_with_data_src');
